...
- **위치**: "거래 기록" 탭, 실현손익 그래프 바로 아래. new1의 §6-17과 UI가 거의 동일 —
  ...
  대비 이겼으면 빨강·졌으면 파랑) / 혼합지수 vs 내 주식 당일 한 줄 / RP 2줄(내 주식·내 계좌,
  누적·5일·당일) / 두 그래프(지수대비계좌 ↔ RP)를 스와이프 캐러셀.
- **RP (relative performance) — new1 §6-17과 동일 (2026-09-02 최종)**:
  구간별 `(Δ내수익 − Δ벤치) / |Δ벤치|`. **0 = 시장과 동일, 양수 = 시장보다 잘함, 음수 = 못함**
  (상승·하락장 한 식). 표시 ±3 clamp. 누적/5일 = 구간별 점수 중앙값(median), 당일 = 마지막
  1구간. UI 라벨 "RP"(예전엔 "민감도"; 원점회귀 베타 → 방향 일관 점수 → 정규화 알파로 확정).
  그래프 y축 [-3.2, 3.2], 기준선 y=0. `me` 컬럼 `상대성과누적/최근/당일` (+계좌*), dict 키는
  하위호환으로 `sens_*`/`acct_sens_*` 유지.
