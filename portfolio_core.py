...
def rebuild_portfolio_from_transactions(tx: pd.DataFrame, initial_capital: float,
    ...
    종목명 기준으로 그 실시간 시세 값을 그대로 이어붙여서, 매매일지를 반영하거나 거래를 손볼
    때마다 그날 거래하지 않은 종목들의 시세가 매수가 기준으로 리셋되는 걸 막는다."""
...
FUND_NAV_HISTORY_FILE = HERE / "fund_nav_history.csv"  # 날짜, 기준가 — VIP한국형가치투자[주식]A-e (new1 §6-21)
...
def load_both_accounts() -> pd.DataFrame:
    ...
    Supabase account_snapshot(런타임 채널)이 공급한다."""
...
def write_account_snapshot(app: str, cum: float, day, total_asset: float,
    ...
    day = 그날 당일 움직임. 시세 새로고침 핸들러에서 호출. 실패해도 조용히 False."""
...
def fetch_peer_account_snapshot(peer_app: str, sb_url: str, sb_key: str) -> dict | None:
    """상대 앱의 최신 계좌 스냅샷(new1 §6-21 런타임 채널). peer_app='orchestra'|'orchestration'.
    반환: {'trade_date','cum','day','total_asset','updated_at'} 또는 None. meritz VIP 패널이
    Orchestra(new1) 최신값을 이걸로 가져와 both_accounts.csv 커밋 지연을 우회한다."""
    if not sb_url or not sb_key:
        return None
    try:
        resp = requests.get(
...
def snapshot_fund_nav_history(nav: float, on_date: str | None = None) -> None:
    ...
    resolve_trading_date 안 씀 — 사용자가 이미 그 거래일 값을 골라 줌 (new1 §6-21)."""
...
# ------------------------------------------------------------------ #
# "SamHynix extracted" (new1 §6-19 포팅) — 코스피 지수에서 대형 반도체(삼성전자/삼성전자우/
# SK하이닉스)를 덜어낸 합성 지수. 이 3종목이 코스피 시총 절반 안팎(2026-09-04 ~52%).
# 혼합지수(국내)의 코스피 다리를 이 합성 지수로 갈아끼워 반도체 안 담는 포트폴리오를
# 더 공정하게 비교.
# ------------------------------------------------------------------ #
BIGCAP_HISTORY_FILE = HERE / "bigcap_history.csv"  # 날짜, 삼성전자, 삼성전자우, SK하이닉스 (종가)
BIGCAP_CODES = {"삼성전자": "005930", "삼성전자우": "005935", "SK하이닉스": "000660"}
# wᵢ(t) = sharesᵢ · closeᵢ(t) / TOTAL(t), TOTAL(t) = TOTAL0 · KOSPI(t)/KOSPI0.
# 상장주식수/TOTAL0(ETF·ETN 제외 추정)는 분기에 한 번 갱신 권장. 스냅샷: 2026-09-04.
...
def synthetic_kospi_ex_bigcap(index_hist: pd.DataFrame, bigcap_hist: pd.DataFrame) -> pd.DataFrame:
    ...
    바꾼 사본. compute_index_vs_account에 그대로 넘기면 코스피 다리·혼합지수·RP가 전부
    '반도체 제외'로 계산된다. 첫날 레벨 = 원본 KOSPI 첫날 값. bigcap_hist 비면 원본 그대로."""
...
def compute_index_vs_account(tx: pd.DataFrame, dom_asset_hist: pd.DataFrame, index_hist: pd.DataFrame,
                              ...
                              kospi_weight: float | None = None, beta_window: int = 5,
    ...
    # 펀드(new1 §6-21): anchor일 기준가 대비 누적등락을 idx_cum 각 날짜에 붙임(그날 이하 가장
    # 최근 기준가 — 발표 하루 밀림 대비). fund_nav_hist 없으면 컬럼 안 만듦.
    ...
    세 선 모두 anchor일(8/14) = 0 리베이스:
      ...
      - 이 앱 '자신의' 계좌(self_key: new1→'orchestra', meritz→'orchestration')
        = 이 앱이 이미 라이브로 계산한 me['계좌수익']을 첫값 대비 재기준화 — Account:Index 패널의
          '내 계좌'와 같은 데이터. both_accounts.csv 동기화 지연에 영향 안 받음("앱에 이미 있는 값").
      - '다른' 계좌 = both_accounts.csv(마감된 날들의 확정 히스토리 + 선그래프 소스).
        **`peer_latest`(Supabase account_snapshot의 상대 앱 최신값, 런타임 채널)가 있으면 표의
        누적/당일은 그 값을 쓰고 선그래프 마지막 점을 그 값으로 이어붙임/덮음** — 상대 앱이 방금
        새로고침한 라이브 값이 커밋 지연 없이 반영된다. 파일·peer 둘 다 없으면 그 선 생략(None).
...
# ------------------------------------------------------------------ #
# P&L Actions (new1 §6-20) — 실현손익을 매매 스타일(FA/MO/MA)로 해부
# ------------------------------------------------------------------ #
...
def compute_pnl_actions(tx: pd.DataFrame, holdings: pd.DataFrame) -> dict:
    ...
    (new1 §6-20). 도넛·상태표·Watering 상세에 그대로 쓰는 값들을 반환."""
    ...
    for c in watering:
        ...
        if px and q and avg and c["first_buy_px"]:
            w_val += q * px
            w_cost += q * avg
            w_firstcost += q * c["first_buy_px"]
            w_seed_first += c["first_buy_qty"] * c["first_buy_px"]
