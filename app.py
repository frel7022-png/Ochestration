...
with tab_tx:
    ...
    def _render_iva_panel(iva, kospi_label):
        """'지수 대비 계좌'(국내만) 한 벌 — 5줄 표 + CR(국면막대) 2줄 + 국면막대 그래프 1장.
        메인(코스피)과 'SamHYnix extracted'(코스피 다리 = 삼성·하이닉스 제외)가 공유한다.
        (2026-09-05, new1과 동일하게 누적 선그래프/캐러셀 제거 — "궁극적으로 보려는 건 누적
        CR 수치고 그래프는 매일매일 오늘 어땠나 보는 용도"라는 사용자 판단.)"""
        ...
        # ---- 국면별 민감도 막대 그래프 (2026-09-05, 내 계좌 기준으로 전환 — new1 §6-17과 동일) ----
        # 하루 = 막대 하나. 시장이 내린 날 = 빨강 막대(그날 "방어" = 1 − 내계좌당일/벤치당일),
        # 오른 날 = 파랑 막대(그날 "참여" = 내계좌당일/벤치당일). 기준선 y=1 = 시장과 똑같이 움직임.
        # 이전엔 내 주식(Rs) 기준이었는데 "실제 운용 중인 계좌 vs 지수"를 매일 보고 싶다는
        # 사용자 판단으로 계좌 기준으로 전환 — 내 주식(Rs) CR은 위 요약 줄에 숫자로 남음.
        # 누적 선그래프도 없앰(위 표로 충분, 그래프는 "오늘" 용도).
        ...
        # 막대 값만 보면 극단치가 왜 그런지 판단이 안 돼서 원본 % 2개를 같이 보여줌 —
        # 혼합지수(벤치) 당일 / 내 계좌 당일.
        _bar_cd = list(zip(
